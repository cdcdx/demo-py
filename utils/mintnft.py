...
from utils.db import get_db_app, format_query_for_db
from utils.i18n import get_text
from utils.local import generate_referral_code
from utils.web3_tools import contract_nftmint
from utils.log import log as logger

# 异步: 直接调用  direct kafka
async def async_boxnft_mintnft(value):
    ...
    async with get_db_app() as cursor:
        ...
        if existing_record is None:
            ...
            nft_count = existing_record['len'] if isinstance(existing_record, dict) else existing_record[0]
        if nft_count > 0:
        ...
        if status == 1:
            await del_redis_data(True, f"box:{userid}:session")
            # 白名单发起NFT铸造
